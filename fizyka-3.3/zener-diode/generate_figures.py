...
def prepare_data(path: str, sort_ascending: Optional[bool] = True) -> pd.DataFrame:
    ...
    uu = [
        "{:.2u}".format(ufloat(u, compute_uncertainty(u, BRYMEN_BM811_VOLTAGE)))
        for u in df["U"]
    ]
    iu = [
        "{:.2u}".format(ufloat(i, compute_uncertainty(i, BRYMEN_BM811_CURRENT)))
        for i in df["I"]
    ]

    df["U"] = [u.split("+")[0] for u in uu]
    df["u(U)"] = [u.split("+/-")[1] for u in uu]

    df["I"] = [i.split("+")[0] for i in iu]
    df["u(I)"] = [i.split("+/-")[1] for i in iu]

    # df["U"] = ["{:.2u}".format(ufloat(u, uncertainty)).split("+")[0] for u, uncertainty in zip(df["U"], df["u(U)"])]
    # df["I"] = ["{:.2u}".format(ufloat(u, uncertainty)).split("+")[0] for u, uncertainty in zip(df["I"], df["u(I)"])]
...
def get_zener_characteristics(
    ...
    print(
        "breakthrough voltage uncertainty",
        sqrt((b / (a**2) * ua) ** 2 + (1 / a * ub) ** 2),
    )
    ...
    u_static_resistance = sqrt(
        (
            (u_zener_voltage / zener_current) ** 2
            + (zener_voltage * u_zener_current) / (zener_current**2)
        )
        ** 2
    )
    ...
    print(
        "min voltage: ",
        min_voltage,
        compute_uncertainty(min_voltage, BRYMEN_BM811_VOLTAGE),
    )
    print(
        "min current: ",
        min_current,
        compute_uncertainty(min_current, BRYMEN_BM811_CURRENT),
    )
    print(
        "zener voltage: ",
        zener_voltage,
        compute_uncertainty(zener_voltage, BRYMEN_BM811_VOLTAGE),
    )
    print(
        "zener current: ",
        zener_current,
        compute_uncertainty(zener_current, BRYMEN_BM811_CURRENT),
    )
...
if __name__ == "__main__":
    matplotlib.use("Agg")
    print(compute_uncertainty(52, BRYMEN_BM811_VOLTAGE))
    for name, forward_polarity, reverse_polarity in ZENER_DIODE_DATA_FILES:
        ...
        formatting = {key: "{:.2f}" for key in rounded.keys()}
        print(formatting)

        rounded.style.format(formatting)
